...
const ALLOWED_ACTIONS: Record<string, string> = {
  ...
  growth: "/admin-ai/growth",
  revenue: "/admin-ai/revenue",
  risks: "/admin-ai/risks",
  recommendations: "/admin-ai/recommendations",
};
      ...
      return NextResponse.json(
        {
          error:
            data?.detail ||
            data?.error ||
            `AdminAI backend failed with status ${backendResponse.status}.`,
        },
    ...
    return NextResponse.json(
      {
        error:
          error?.message ||
          "AdminAI request failed. Check backend deployment and NEXT_PUBLIC_BACKEND_URL.",
      },
      { status: 500 }
    );
  }
