...
export default function VideoInterviewIQPage() {
  ...
  function downloadInterviewReport() {
    if (!analysis) {
      setMessage("Please analyze interview before downloading report.");
      return;
    }
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(18);
    doc.text("Chumcred InterviewIQ Report", margin, y);
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(16);
    doc.text(`Overall Score: ${analysis.overall_score || 0}%`, margin, y);

    y += 10;
    doc.setFont("helvetica", "normal");
    doc.setFontSize(10);
    doc.text(`Communication: ${analysis.communication_score || 0}%`, margin, y);
    doc.text(`Confidence: ${analysis.confidence_score || 0}%`, margin + 55, y);
    doc.text(`Technical: ${analysis.technical_score || 0}%`, margin + 105, y);

    y += 6;
    doc.text(`Structure: ${analysis.structure_score || 0}%`, margin, y);
    doc.text(
      `Professionalism: ${analysis.professionalism_score || 0}%`,
      margin + 55,
      y
    );

    y += 12;
    y = addWrappedText(doc, "Interview Question", question, margin, y, maxWidth);
    ...
    y = addWrappedText(doc, "Candidate Transcript", transcript, margin, y, maxWidth);
    ...
    y = addWrappedText(doc, "Strengths", analysis.strengths || "", margin, y, maxWidth);
    ...
    doc.setFontSize(9);
    doc.text(
      "Generated by Chumcred InterviewIQ | jobs.chumcred.com",
      margin,
      285
    );

    doc.save(`InterviewIQ_Report_${Date.now()}.pdf`);
  }
